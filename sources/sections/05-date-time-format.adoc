...
precise, then a useful property is achieved. Assuming that the UTC
offsets of the dates and times are the same (e.g., all in UTC),
expressed using the same string (e.g., all "Z" or all "+00:00"), all
times have the same number of fractional second digits, all years are
in the same era and use the same number of digits, and all the dates and
times have the same suffix, then they may be sorted as strings (e.g.,
using the `strcmp()` function in C) and a time-ordered sequence will
result (in descending order if the years start with "-"). The presence
of optional punctuation would violate this characteristic.
